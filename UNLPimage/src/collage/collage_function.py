...
from UNLPimage.common.path import PATH
import os
import PIL.Image
import PIL.ImageTk
import PIL.ImageOps
...
def open_csv():
    ...
    - Verifica que las imagenes de metadata existan y esten disponibles
    para ser utilizadas.
    - En caso que haya mas de una entrada para la misma foto en metadata,
    muestra el nombre una unica vez.
    ...
    try:
        ...
        list_relative_path = list(
            set([element["relative_path"] for element in data_csv])
        )
        filtered_path = filter(
            lambda x: os.path.isfile(os.path.join(PATH, x)), list_relative_path
        )
        list_names_images = [os.path.basename(path) for path in filtered_path]
