...
def open_record() -> dict:
    ...
    try:
        ...
        try:
            ...
            sg.popup(
                """ERROR: excepcion MemoryError,
                no se pudo crear el archivo json de rutas"""
            )
        except PermissionError:
            sg.popup(
                """ERROR: excepcion PermissionError,
                no se pudo crear el archivo json de rutas"""
            )
    else:
...
def try_open_csv(header: list[str], file_name="metadata.csv"):
    ...
    Args:
        ...
        file_name (str, optional): el nombre del archivo a abrir.
    ...
    try:
        ...
        try:
            ...
            sg.popup(
                f"""ERROR: excepcion MemoryError,
                     no se pudo crear el {file_name}"""
            )
        except PermissionError:
            sg.popup(
                f"""ERROR: excepcion PermissionError,
                no se pudo crear el {file_name}"""
            )
...
def election(copy, title, path):
    ...
    Return:
        _boolean_ : confirma si pudo o no guardar la imagen
    """
    confirmation = False
    layout = [
    ...
    if (event == "-JPG-") or (event == "-PNG-"):
        confirmation = save_image(copy, title, path, extension)
    return confirmation


def save_image(copy, title, path, extension):
    """Funcion que Guarda una imagen en formato png o jpg.
    - En caso que exista una imagen con ese nombre consulta si desea
        sobreescribirla
    ...
    Return _boolean_ : confirma si pudo o no guardar la imagen
    """
    confirmation = False
    route = os.path.join(path, title + extension)
    if not os.path.isfile(os.path.join(PATH, route)):
        if extension == ".png":
        ...
        answer = sg.popup_yes_no(
            """Existe una imagen con ese nombre,
