...
def take_memes():
    ...
    """
    try:
        memes_data = open_json_dict("memes.json")
    ...
    names = [elem["name"] for elem in memes_data if os.path.isfile(os.path.join(PATH_DEFAULT_MEMES, elem["image"]))]
    return names
...
def save_meme(copy, path, meme_info, text):
    ...
    if title != None and len(title.strip()) <= 30 and len(title.strip()) > 3:
        confirmation = election(copy, title, path)
        if confirmation:
            logs.Log.write_log("generó meme", meme_info["image"], text)
    else:
